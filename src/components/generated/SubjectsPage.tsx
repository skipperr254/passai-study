...
export const SubjectsPage = (props: SubjectsPageProps) => {
  ...
  return <div className="h-full overflow-y-auto pb-4">
      ...
      <div className="px-4 py-4 lg:px-8 lg:py-6 bg-gradient-to-br from-slate-50 to-blue-50/30 border-b border-slate-200/60">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-start justify-between mb-4 lg:mb-6">
            <div>
              <h1 className="text-2xl lg:text-4xl font-bold text-slate-900 mb-1 lg:mb-2">My Subjects</h1>
              <p className="text-sm lg:text-base text-slate-600">Manage and track your learning progress</p>
            </div>
            <button onClick={() => setIsAddSubjectOpen(true)} className="hidden lg:flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-semibold rounded-xl hover:shadow-lg active:scale-95 transition-all">
          ...
          <div className="grid grid-cols-4 gap-2 lg:gap-4 mb-4">
            <button onClick={() => setFilterBy('all')} className={`bg-white rounded-xl p-3 lg:p-4 border-2 transition-all active:scale-95 ${filterBy === 'all' ? 'border-blue-500 ring-2 ring-blue-100' : 'border-slate-200'}`}>
            ...
            <button onClick={() => setFilterBy('needs-attention')} className={`bg-white rounded-xl p-3 lg:p-4 border-2 transition-all active:scale-95 ${filterBy === 'needs-attention' ? 'border-red-500 ring-2 ring-red-100' : 'border-slate-200'}`}>
            ...
            <button onClick={() => setFilterBy('on-track')} className={`bg-white rounded-xl p-3 lg:p-4 border-2 transition-all active:scale-95 ${filterBy === 'on-track' ? 'border-green-500 ring-2 ring-green-100' : 'border-slate-200'}`}>
            ...
            <button onClick={() => setFilterBy('excelling')} className={`bg-white rounded-xl p-3 lg:p-4 border-2 transition-all active:scale-95 ${filterBy === 'excelling' ? 'border-emerald-500 ring-2 ring-emerald-100' : 'border-slate-200'}`}>
      ...
      <div className="px-4 py-4 lg:px-8 lg:py-6">
        <div className="max-w-7xl mx-auto">
          {filteredSubjects.length === 0 ? <div className="text-center py-12">
            ...
            </div> : <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 lg:gap-4">
            ...
            return <div key={subject.id} onClick={() => {
              ...
              props.onSubjectClick?.(subject.id);
                    ...
                    <div className="flex items-start gap-3 mb-4">
                      <div className={`w-12 h-12 lg:w-14 lg:h-14 rounded-xl bg-gradient-to-br ${subject.color} flex items-center justify-center shadow-md flex-shrink-0`}>
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-3">
                      <div className={`p-2 lg:p-3 rounded-lg ${getPassingChanceColor(subject.passingChance)}`}>
                      ...
                      <div className="p-2 lg:p-3 rounded-lg bg-slate-50 border border-slate-200">
                        <div className="flex items-center gap-1.5 mb-1">
                          ...
                          <span className="text-[10px] lg:text-xs font-semibold text-slate-600">Avg Score</span>
                        </div>
                        <p className="text-lg lg:text-xl font-bold text-slate-900">{subject.averageScore}%</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-slate-200">
                      ...
                      {subject.upcomingQuiz && <div className="flex items-center gap-1 px-2 py-1 bg-blue-50 text-blue-600 rounded-lg">
      ...
      {selectedSubject && <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200" onClick={() => setSelectedSubject(null)}>
          <div className="bg-white rounded-t-3xl md:rounded-2xl shadow-2xl w-full md:max-w-3xl max-h-[85vh] flex flex-col animate-in slide-in-from-bottom md:zoom-in-95 duration-300" onClick={e => e.stopPropagation()}>
            {/* Modal Header */}
            <div className={`flex-shrink-0 px-6 py-5 border-b border-slate-200 bg-gradient-to-br ${selectedSubject.color}`}>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-white/90 text-sm">{selectedSubject.studyHours} hours studied • {selectedSubject.totalMaterials} materials</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6">
              ...
              <div className="grid grid-cols-3 gap-3 mb-6">
                <div className="text-center p-4 bg-slate-50 rounded-xl border border-slate-200">
                  <p className="text-2xl font-bold text-slate-900 mb-1">{selectedSubject.progress}%</p>
                  <p className="text-xs text-slate-600 font-medium">Progress</p>
                </div>
                <div className={`text-center p-4 rounded-xl ${getPassingChanceColor(selectedSubject.passingChance)}`}>
                  <p className="text-2xl font-bold mb-1">{selectedSubject.passingChance}%</p>
                  <p className="text-xs font-medium">Passing</p>
                </div>
                <div className="text-center p-4 bg-slate-50 rounded-xl border border-slate-200">
                  <p className="text-2xl font-bold text-slate-900 mb-1">{selectedSubject.averageScore}%</p>
              ...
              <div className="mb-6 p-4 bg-blue-50 border-2 border-blue-200 rounded-xl">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm font-bold text-slate-900">{selectedSubject.nextMilestone}</p>
                  </div>
              ...
              <div className="grid md:grid-cols-2 gap-4 mb-6">
                ...
                <div className="p-4 bg-emerald-50 border-2 border-emerald-200 rounded-xl">
                  ...
                  <div className="space-y-2">
                    {selectedSubject.strongTopics.map((topic, idx) => <div key={idx} className="flex items-center gap-2">
                ...
                <div className="p-4 bg-amber-50 border-2 border-amber-200 rounded-xl">
                  ...
                  <div className="space-y-2">
                    {selectedSubject.weakTopics.map((topic, idx) => <div key={idx} className="flex items-center gap-2">
      ...
      {/* Mobile FAB */}
      <button onClick={() => setIsAddSubjectOpen(true)} className="lg:hidden fixed bottom-20 right-4 z-30 w-14 h-14 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-full shadow-xl flex items-center justify-center hover:shadow-2xl active:scale-95 transition-all">
        <Plus className="w-6 h-6" />
      </button>

      {/* Add Subject Modal */}
      {isAddSubjectOpen && <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200" onClick={() => setIsAddSubjectOpen(false)}>
          <div className="bg-white rounded-t-3xl md:rounded-2xl shadow-2xl w-full md:max-w-2xl max-h-[90vh] flex flex-col animate-in slide-in-from-bottom md:zoom-in-95 duration-300" onClick={e => e.stopPropagation()}>
            {/* Modal Header */}
            <div className="flex-shrink-0 px-6 py-5 border-b border-slate-200 bg-gradient-to-br from-blue-500 to-indigo-600">
              ...
              <div className="flex items-center gap-4">
                ...
                <button onClick={() => setIsAddSubjectOpen(false)} className="w-9 h-9 rounded-lg bg-white/20 hover:bg-white/30 backdrop-blur-sm flex items-center justify-center transition-all" aria-label="Close modal">
            ...
            <div className="flex-1 overflow-y-auto p-6">
              <div className="space-y-5">
                {/* Subject Name */}
                <div>
                  <label htmlFor="subject-name" className="block text-sm font-bold text-slate-900 mb-2">
                    Subject Name <span className="text-red-500">*</span>
                  </label>
                  <input id="subject-name" type="text" value={newSubject.name} onChange={e => setNewSubject({
                ...newSubject,
                name: e.target.value
              })} placeholder="e.g., Mathematics, History, Biology" className="w-full px-4 py-3 bg-white border-2 border-slate-200 rounded-xl text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-100 outline-none transition-all" required />
                </div>

                {/* Description (Optional) */}
                <div>
                  <label htmlFor="subject-description" className="block text-sm font-bold text-slate-900 mb-2">
                    Description <span className="text-slate-400 font-normal">(Optional)</span>
                  </label>
                  <textarea id="subject-description" value={newSubject.description} onChange={e => setNewSubject({
                ...newSubject,
                description: e.target.value
              })} placeholder="Add any additional context about this subject..." rows={3} className="w-full px-4 py-3 bg-white border-2 border-slate-200 rounded-xl text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-100 outline-none transition-all resize-none" />
                </div>

                {/* Test Date (Optional) */}
                <div>
                  <label htmlFor="test-date" className="block text-sm font-bold text-slate-900 mb-2">
                ...
                testDate: e.target.value
                  ...
                  <p className="text-xs text-slate-500 mt-1.5">Set a target date for your upcoming test or exam</p>
                </div>

                {/* Teacher Emphasis */}
                <div>
                  <label htmlFor="teacher-emphasis" className="block text-sm font-bold text-slate-900 mb-2">
                    <div className="flex items-center gap-2">
              ...
              })} placeholder="What does your teacher emphasize? What topics are most important?" rows={4} className="w-full px-4 py-3 bg-white border-2 border-slate-200 rounded-xl text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-100 outline-none transition-all resize-none" />
            ...
            <div className="flex-shrink-0 px-6 py-4 border-t border-slate-200 bg-slate-50">
              <div className="flex gap-3">
                <button onClick={() => setIsAddSubjectOpen(false)} className="flex-1 px-4 py-3 bg-white border-2 border-slate-200 text-slate-700 font-semibold rounded-xl hover:bg-slate-50 active:scale-95 transition-all">
                  Cancel
                </button>
                <button onClick={handleAddSubject} disabled={!newSubject.name.trim()} className="flex-1 px-4 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-semibold rounded-xl hover:shadow-lg active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:active:scale-100">
                  Add Subject
                </button>
              </div>
            </div>
