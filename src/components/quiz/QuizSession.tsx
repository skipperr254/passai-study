...
export const QuizSession = (props: QuizSessionProps) => {
  ...
  // Show results page
  if (isQuizComplete) {
    return <QuizResultsPage quizTitle={props.quizTitle} subject={props.subject} subjectColor={props.subjectColor} results={results} questions={questions} onExit={props.onExit} />;
  }
  return <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/40 pb-20 lg:pb-8">
      {/* Header */}
      <div className={`px-4 py-3 lg:px-8 lg:py-4 bg-gradient-to-br ${props.subjectColor} border-b border-white/20 sticky top-0 z-30`}>
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center justify-between mb-3">
            <button onClick={props.onExit} className="flex items-center gap-2 text-white/90 hover:text-white transition-colors text-sm font-semibold">
              <ChevronLeft className="w-4 h-4" />
              <span className="hidden sm:inline">Exit Quiz</span>
            </button>
            
            <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg ${getTimeColor()} border-2 border-current`}>
      ...
      <div className="max-w-4xl mx-auto px-4 py-6 lg:px-8">
        <div className="bg-white rounded-2xl border-2 border-slate-200 shadow-lg p-5 lg:p-8">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            {!hasSubmitted && <div className="flex items-center gap-1">
                <button onClick={() => handleQuestionFeedback('thumbs-up')} className="w-8 h-8 rounded-lg hover:bg-slate-100 flex items-center justify-center transition-colors group" title="Good question">
                  <ThumbsUp className="w-4 h-4 text-slate-400 group-hover:text-emerald-600" />
                </button>
                <button onClick={() => handleQuestionFeedback('thumbs-down')} className="w-8 h-8 rounded-lg hover:bg-slate-100 flex items-center justify-center transition-colors group" title="Report issue">
          ...
          <div className="space-y-3 mb-6">
            ...
            return <button key={idx} onClick={() => !hasSubmitted && setSelectedAnswer(option)} disabled={hasSubmitted} className={`w-full text-left px-4 lg:px-5 py-3 lg:py-4 rounded-xl font-semibold transition-all active:scale-[0.98] ${optionClass} ${hasSubmitted ? 'cursor-default' : 'cursor-pointer'}`}>
                  <div className="flex items-center gap-3">
                    ...
                    {showResult && isCorrect && <CheckCircle2 className="w-5 h-5 text-emerald-600 flex-shrink-0" />}
          ...
          {hasSubmitted && <div className={`p-4 lg:p-5 rounded-xl border-2 mb-6 ${results[results.length - 1]?.isCorrect ? 'bg-emerald-50 border-emerald-200' : results[results.length - 1]?.wasAnswered ? 'bg-red-50 border-red-200' : 'bg-amber-50 border-amber-200'}`}>
              <div className="flex items-start gap-3 mb-3">
                {results[results.length - 1]?.isCorrect ? <CheckCircle2 className="w-6 h-6 text-emerald-600 flex-shrink-0" /> : results[results.length - 1]?.wasAnswered ? <XCircle className="w-6 h-6 text-red-600 flex-shrink-0" /> : <AlertCircle className="w-6 h-6 text-amber-600 flex-shrink-0" />}
                <div className="flex-1">
                  <h3 className="font-bold text-slate-900 mb-1">
                    {results[results.length - 1]?.isCorrect ? '✓ Correct!' : results[results.length - 1]?.wasAnswered ? '✗ Incorrect' : '⏱ Time\'s Up!'}
                  </h3>
                  {!results[results.length - 1]?.isCorrect && <p className="text-sm font-semibold text-slate-700 mb-2">
          ...
          <button onClick={() => setShowSourceModal(true)} className="w-full mb-6 p-3 lg:p-4 bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-xl transition-colors text-left group">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm font-bold text-slate-900 truncate">{currentQuestion.sourceSnippet.material}</p>
                <p className="text-xs text-slate-600 mt-1">Page {currentQuestion.sourceSnippet.page} • Click to review</p>
              </div>
          ...
          <div className="flex gap-3">
            {!hasSubmitted ? <button onClick={handleSubmitAnswer} disabled={!selectedAnswer} className="flex-1 flex items-center justify-center gap-2 px-6 py-3 lg:py-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-bold rounded-xl hover:shadow-lg active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:active:scale-100">
              ...
              </button> : <button onClick={handleNextQuestion} className="flex-1 flex items-center justify-center gap-2 px-6 py-3 lg:py-4 bg-gradient-to-r from-emerald-600 to-green-600 text-white font-bold rounded-xl hover:shadow-lg active:scale-95 transition-all">
                <span>{isLastQuestion ? 'View Results' : 'Next Question'}</span>
                {isLastQuestion ? <Flag className="w-5 h-5" /> : <ChevronRight className="w-5 h-5" />}
              </button>}
          </div>
        </div>
      </div>
      ...
      {showSourceModal && <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200" onClick={() => setShowSourceModal(false)}>
          <div className="bg-white rounded-t-3xl md:rounded-2xl shadow-2xl w-full md:max-w-2xl max-h-[80vh] flex flex-col animate-in slide-in-from-bottom md:zoom-in-95 duration-300" onClick={e => e.stopPropagation()}>
            <div className="flex-shrink-0 px-6 py-4 border-b border-slate-200">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-slate-900">{currentQuestion.sourceSnippet.material}</h3>
                    <p className="text-xs text-slate-600">Page {currentQuestion.sourceSnippet.page}</p>
                  </div>
                </div>
                <button onClick={() => setShowSourceModal(false)} className="w-9 h-9 rounded-lg bg-slate-100 hover:bg-slate-200 flex items-center justify-center transition-colors">
                  <X className="w-5 h-5" />
                </button>
              </div>
            </div>
      ...
      {/* Mood Check Modal */}
      {showMoodCheck && <MoodCheckModal onComplete={() => setShowMoodCheck(false)} currentScore={Math.round(results.filter(r => r.isCorrect).length / results.length * 100)} />}
    </div>;
};