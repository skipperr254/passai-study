...
type MoodCheckModalProps = {
  onComplete: (mood: Mood, energyLevel: number) => void;
  currentScore: number;
};
...
export const MoodCheckModal = (props: MoodCheckModalProps) => {
  ...
  const handleContinue = () => {
    if (selectedMood) {
      // Send mood data and energy level to backend for adaptive quiz adjustment
      props.onComplete(selectedMood, energyLevel);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-300 p-4 overflow-y-auto">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg animate-in zoom-in-95 duration-300 my-8 max-h-[calc(100vh-4rem)] flex flex-col">
        {/* Header */}
        <div className="flex-shrink-0 px-6 lg:px-8 pt-8 pb-6 border-b border-slate-200 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-t-3xl">
        ...
        <div className="flex-1 overflow-y-auto px-6 lg:px-8 py-6">
          {/* Current Performance - Commented out for cleaner UI, but kept for future use */}
        ...
        <div className="flex-shrink-0 px-6 lg:px-8 pb-8 pt-4 border-t border-slate-100">
          <button
