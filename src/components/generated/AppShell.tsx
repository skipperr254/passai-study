...
export const AppShell = (props: AppShellProps) => {
  ...
  const renderPage = () => {
    switch (activeNav) {
      case 'subjects':
        return <SubjectsPage onSubjectClick={id => setActiveNav('dashboard')} />;
      case 'quizzes':
        return <QuizzesPage selectedSubjectId={selectedSubjectId} onQuizClick={id => console.log('Quiz clicked:', id)} />;
      case 'upload':
        return <MaterialsPage preSelectedSubjectId={selectedSubjectId !== 'all' ? selectedSubjectId : undefined} />;
      case 'study-plan':
        return <StudyPlanPage preSelectedSubjectId={selectedSubjectId !== 'all' ? selectedSubjectId : undefined} />;
      case 'dashboard':
        return <DashboardPage userName={userName} />;
      default:
        return props.children || <DashboardPage userName={userName} />;
    }
  };
      ...
      <div className="flex flex-col h-screen lg:flex-row">
        ...
        <aside className="hidden lg:flex w-80 bg-white/80 backdrop-blur-xl border-r border-slate-200/60 flex-col shadow-sm">
          ...
          <div className="p-4 border-t border-slate-200/60 space-y-2">
            ...
            <button onClick={props.onSettingsClick} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-slate-700 hover:bg-slate-50 transition-all">
        ...
        <header className="lg:hidden fixed top-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-xl border-b border-slate-200/60 shadow-sm safe-top">
          <div className="px-4 py-3">
            <div className="flex items-center justify-between">
              <button onClick={props.onProfileClick} className="flex items-center gap-3 hover:opacity-80 transition-opacity active:scale-95">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={props.onSettingsClick} className="w-9 h-9 rounded-lg bg-slate-100 hover:bg-slate-200 active:scale-95 flex items-center justify-center transition-all">
