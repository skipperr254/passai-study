...
import type { SubjectPerformance } from '@/services/dashboard.service';

type DashboardPageProps = {
  userName?: string;
};

const mockQuizData_OLD = [
  {
    quizNumber: 1,
    score: 65,
  },
...
export const DashboardPage = (props: DashboardPageProps) => {
  ...
  const { stats, subjectPerformance, progressData, loading } = useDashboard(selectedSubjectId);
  const [isSubjectModalOpen, setIsSubjectModalOpen] = useState(false);

  // Set initial selected subject
  useEffect(() => {
    if (!selectedSubjectId && subjectPerformance.length > 0) {
      setSelectedSubjectId(subjectPerformance[0].subjectId);
    }
  }, [subjectPerformance, selectedSubjectId]);

  const selectedPerformance =
    subjectPerformance.find(p => p.subjectId === selectedSubjectId) || subjectPerformance[0];

  // Calculate derived stats
  const maxScore = progressData.length > 0 ? Math.max(...progressData.map(d => d.score)) : 0;
  const currentScore = progressData.length > 0 ? progressData[progressData.length - 1].score : 0;
  const previousScore =
    progressData.length > 1 ? progressData[progressData.length - 2].score : currentScore;
  ...
  if (!selectedPerformance || subjectPerformance.length === 0) {
    return (
      <div className="h-full flex items-center justify-center p-4">
        <div className="text-center max-w-md">
          ...
          <p className="text-slate-600 mb-4">
            Start by creating subjects and taking quizzes to see your dashboard analytics.
          </p>
        </div>
      </div>
    );
    ...
    <div className="h-full overflow-y-auto">
      <div className="max-w-7xl mx-auto px-4 py-4 sm:px-6 sm:py-6 lg:p-8">
        ...
        <div className="lg:hidden mb-4 -mx-4 px-4 overflow-x-auto hide-scrollbar">
          <div className="flex gap-2 pb-1">
            {subjectPerformance.map(perf => (
              <button
                key={perf.subjectId}
                onClick={() => setSelectedSubjectId(perf.subjectId)}
                className={`flex-shrink-0 px-3 py-2 rounded-xl transition-all active:scale-95 ${selectedSubjectId === perf.subjectId ? `bg-gradient-to-r ${perf.subjectColor} text-white shadow-md` : 'bg-white border-2 border-slate-200 text-slate-700'}`}
              >
                <div className="flex items-center gap-2">
                  ...
                  {selectedSubjectId === perf.subjectId && (
        ...
        <header className="hidden lg:block mb-8">
          <div className="flex items-start justify-between gap-4 mb-4">
            <div className="flex-1">
              <h2 className="text-4xl font-bold text-slate-900 mb-2">
                {selectedPerformance.subjectName}
              </h2>
            ...
            >
              <div
                className={`w-10 h-10 rounded-lg bg-gradient-to-br ${selectedPerformance.subjectColor} flex items-center justify-center shadow-sm`}
              >
              ...
              <div className="flex-1 text-left min-w-0">
                ...
                <p className="text-xs text-slate-600 font-medium">
                  {selectedPerformance.quizzesTaken} quizzes taken
                </p>
              </div>
              <div
                className={`inline-flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-bold ${getPassingChanceColor(selectedPerformance.passingChance)}`}
              >
                <Target className="w-3 h-3" />
                <span>{selectedPerformance.passingChance}%</span>
              </div>
            </button>
          </div>
          ...
          <div
            className={`rounded-2xl p-6 border-2 bg-gradient-to-br ${getPassingChanceBg(selectedPerformance.passingChance)}`}
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div
                  className={`w-14 h-14 rounded-xl flex items-center justify-center shadow-lg bg-gradient-to-br ${getPassingChanceGradient(selectedPerformance.passingChance)}`}
                >
                ...
                <div>
                  ...
                  <div className="flex items-baseline gap-3">
                    <p className="text-5xl font-bold text-slate-900">
                      {selectedPerformance.passingChance}%
                    </p>
                    <span
                      className={`px-3 py-1 rounded-lg text-sm font-bold text-white ${getPassingChanceBadgeBg(selectedPerformance.passingChance)}`}
                    >
                      {getPassingChanceBadge(selectedPerformance.passingChance)}
                    </span>
                    <div className="flex items-center gap-1 text-sm">
                      {selectedPerformance.recentTrend === 'improving' && (
                        <>
                          <TrendingUp className="w-4 h-4 text-emerald-600" />
                          <span className="text-emerald-600 font-semibold">Improving</span>
                        </>
                      )}
                      {selectedPerformance.recentTrend === 'declining' && (
                        <>
                          <TrendingDown className="w-4 h-4 text-red-600" />
                          <span className="text-red-600 font-semibold">Declining</span>
                        </>
                      )}
                      {selectedPerformance.recentTrend === 'stable' && (
                        <>
              ...
              <div className="text-right">
                ...
                <p className="text-2xl font-bold text-slate-900">
                  {selectedPerformance.quizzesTaken} Quizzes
                </p>
                <p className="text-sm text-slate-600 mt-1">
                  Avg: <span className="font-bold">{selectedPerformance.averageScore}%</span>
                </p>
              </div>
            </div>
          </div>
        ...
        <div
          className={`lg:hidden rounded-2xl p-4 mb-4 border-2 bg-gradient-to-br ${getPassingChanceBg(selectedPerformance.passingChance)}`}
        >
          <div className="flex items-start gap-3 mb-3">
            <div
              className={`w-12 h-12 rounded-xl flex items-center justify-center shadow-md bg-gradient-to-br ${getPassingChanceGradient(selectedPerformance.passingChance)}`}
            >
            ...
            <div className="flex-1">
              ...
              <div className="flex items-baseline gap-2">
                ...
                <span
                  className={`px-2 py-1 rounded-lg text-xs font-bold text-white ${getPassingChanceBadgeBg(selectedPerformance.passingChance)}`}
                >
                  {getPassingChanceBadge(selectedPerformance.passingChance)}
                </span>
              </div>
          ...
          <div className="flex items-center justify-between text-sm">
            <span className="text-slate-600">
              Based on{' '}
              <span className="font-bold text-slate-900">{selectedPerformance.quizzesTaken}</span>{' '}
              quizzes
            </span>
            <span className="text-slate-600">
              Avg:{' '}
              <span className="font-bold text-slate-900">{selectedPerformance.averageScore}%</span>
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-4 lg:grid-cols-4 lg:gap-4 lg:mb-8">
          <div className="bg-white/90 backdrop-blur-sm rounded-xl lg:rounded-2xl p-3 lg:p-6 border border-slate-200/60 shadow-sm active:scale-[0.98] lg:hover:shadow-md transition-all">
            <div className="flex flex-col lg:flex-row lg:items-center gap-2 lg:gap-3 mb-2 lg:mb-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs lg:text-sm text-slate-600 font-medium">Latest Score</p>
            ...
            <div className="flex items-center gap-1.5 text-xs lg:text-sm">
              {scoreDiff > 0 && (
                <>
                  <span className="text-emerald-600 font-bold">+{scoreDiff}%</span>
                  <span className="text-slate-500">from last</span>
                </>
              )}
              {scoreDiff < 0 && (
                <>
                  <span className="text-red-600 font-bold">{scoreDiff}%</span>
                  <span className="text-slate-500">from last</span>
                </>
              )}
              {scoreDiff === 0 && <span className="text-slate-500">No change</span>}
            </div>
          </div>
          ...
          <div className="bg-white/90 backdrop-blur-sm rounded-xl lg:rounded-2xl p-3 lg:p-6 border border-slate-200/60 shadow-sm active:scale-[0.98] lg:hover:shadow-md transition-all">
            <div className="flex flex-col lg:flex-row lg:items-center gap-2 lg:gap-3 mb-2 lg:mb-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs lg:text-sm text-slate-600 font-medium">Streak</p>
          ...
          <div className="bg-white/90 backdrop-blur-sm rounded-xl lg:rounded-2xl p-3 lg:p-6 border border-slate-200/60 shadow-sm active:scale-[0.98] lg:hover:shadow-md transition-all">
            <div className="flex flex-col lg:flex-row lg:items-center gap-2 lg:gap-3 mb-2 lg:mb-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs lg:text-sm text-slate-600 font-medium">Study Time</p>
                <p className="text-2xl lg:text-3xl font-bold text-slate-900">
                  {formatTime(stats?.totalTimeStudied || 0)}
                </p>
              </div>
            </div>
            <p className="text-xs lg:text-sm text-slate-500 font-medium truncate">Total</p>
          </div>
        </div>
          ...
          <div className="relative h-40 lg:h-64">
            ...
            >
              ...
              {progressData.length > 0 && (
                <>
                  <path
                    d={`M 0,${200 - progressData[0].score * 2} ${progressData.map((d, i) => `L ${(i * 800) / (progressData.length - 1)},${200 - d.score * 2}`).join(' ')} L 800,200 L 0,200 Z`}
                    fill="url(#scoreGradient)"
                  />

                  <path
                    d={`M 0,${200 - progressData[0].score * 2} ${progressData.map((d, i) => `L ${(i * 800) / (progressData.length - 1)},${200 - d.score * 2}`).join(' ')}`}
                    fill="none"
                  ...
                  {progressData.map((d, i) => (
                    <g key={i}>
                      <circle
                        cx={(i * 800) / (progressData.length - 1)}
                        cy={200 - d.score * 2}
                        r="6"
                        fill="white"
                        stroke="url(#lineGradient)"
              ...
              {progressData.length === 0 && (
                <text x="400" y="100" textAnchor="middle" className="text-sm fill-slate-400">
                  No quiz data yet
                </text>
              )}
            </svg>
        ...
        {selectedPerformance.upcomingQuiz && (
          <section
            ...
            <div className="flex flex-col lg:flex-row items-start justify-between gap-4 lg:gap-6">
              <div className="flex-1">
                ...
                <p className="text-base lg:text-lg font-semibold mb-4 lg:mb-6 text-white/95">
                  {new Date(selectedPerformance.upcomingQuiz).toLocaleString('en-US', {
                    weekday: 'long',
                    month: 'long',
                    day: 'numeric',
                    hour: 'numeric',
              ...
              <div className="w-full lg:w-auto bg-white/10 backdrop-blur-sm rounded-xl lg:rounded-2xl p-4 lg:p-6 border border-white/20 lg:min-w-[200px]">
                <p className="text-xs lg:text-sm text-blue-100 mb-2 font-medium">Time Until Quiz</p>
                <p className="text-3xl lg:text-4xl font-bold mb-1">
                  {Math.ceil(
                    (new Date(selectedPerformance.upcomingQuiz).getTime() - Date.now()) /
                      (1000 * 60 * 60 * 24)
                  )}
                </p>
                <p className="text-xs lg:text-sm text-blue-100 font-medium">days</p>
              </div>
            </div>
          </section>
        )}
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4 md:px-6">
              <div className="space-y-2 md:space-y-3">
                {subjectPerformance.map(perf => {
                  const isSelected = selectedSubjectId === perf.subjectId;
                  return (
                    <button
                      key={perf.subjectId}
                      onClick={() => {
                        setSelectedSubjectId(perf.subjectId);
                        setIsSubjectModalOpen(false);
                      }}
                      className={`w-full bg-white rounded-xl border-2 p-3 md:p-4 text-left transition-all active:scale-[0.98] ${isSelected ? 'border-blue-500 shadow-md ring-2 ring-blue-100' : 'border-slate-200 hover:border-slate-300'}`}
                    >
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-12 h-12 flex-shrink-0 rounded-xl bg-gradient-to-br ${perf.subjectColor} flex items-center justify-center shadow-sm`}
                        >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-2">
                            <div className="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                              <div
                                className={`h-full bg-gradient-to-r ${perf.subjectColor} rounded-full transition-all duration-500`}
                        ...
                        <div
                          className={`px-2.5 py-1.5 rounded-lg text-xs font-bold ${getPassingChanceColor(perf.passingChance)} flex-shrink-0`}
                        >
                          <div className="flex items-center gap-1">
                            <Target className="w-3 h-3" />
                            <span>{perf.passingChance}%</span>
                          </div>
                        </div>
                      </div>
                    </button>
